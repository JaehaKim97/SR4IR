...
def make_model(opt):
    return HRSegmentationModel(opt)


class HRSegmentationModel(BaseModel):
    """Semantic Segmentation model using HR images."""
    ...
    def __init__(self, opt):
        ...
        self.net_seg = self.model_to_device(self.net_seg, is_trainable=True)
    ...
    def train_one_epoch(self, data_loader_train, train_sampler, epoch):
        ...
        for iter, (img_hr, target) in enumerate(metric_logger.log_every(data_loader_train, self.opt['print_freq'], self.text_logger, header)):
            ...
            if hasattr(self, 'cri_auxce'):
                l_auxce = self.cri_auxce(pred_hr, target)
            ...
            metric_logger.update(lr_seg=self.optimizer_seg.param_groups[0]["lr"])
    ...
    def evaluate(self, data_loader_test, epoch=0, num_classes=21):
        ...
        for (img_hr, target), filename in metric_logger.log_every(data_loader_test, 500, self.text_logger, header, return_filename=True):
            ...
            if self.opt['test'].get('visualize', False): # and (num_processed_samples < 20):
                seg_map_gt = self.convert2color(target)
                visualize_image_from_batch(seg_map_gt.unsqueeze(0), osp.join(self.exp_dir, 'visualize'), [filename.replace('.jpg', '_seg_gt.jpg')])
                
                seg_map_pred = self.convert2color(pred_hr['out'].argmax(1))
                visualize_image_from_batch(seg_map_pred.unsqueeze(0), osp.join(self.exp_dir, 'visualize'), [filename.replace('.jpg', '_seg_pred.jpg')])
                
                visualize_image_from_batch(img_hr, osp.join(self.exp_dir, 'visualize'), [filename])
            
            # evaluation on validation batch
            confmat += calculate_mat(target.flatten(), pred_hr['out'].argmax(1).flatten(), n=num_classes)
        ...
        metric_summary = self.add_metric(metric_summary, 'mean IoU-HR', iu.mean().item() * 100, epoch)
